...
async def create_blog(
    payload: BlogIn, db: AsyncSession = Depends(get_db), user: User = Depends(_can_manage)
):
    ...
    db.add(
        AuditLog(
            actor=user.username, action="blog-add", message=f"Blog yazısı eklendi: {payload.title}"
        )
    )
    await db.commit()
    await db.refresh(p)
...
async def update_blog(
    ...
    db.add(
        AuditLog(
            actor=user.username,
            action="blog-edit",
            message=f"Blog yazısı güncellendi: {p.title} (#{p.id})",
        )
    )
    await db.commit()
    await db.refresh(p)
...
async def delete_blog(
    post_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(_can_manage)
):
    p = (await db.execute(select(BlogPost).where(BlogPost.id == post_id))).scalar_one_or_none()
    if not p:
        raise HTTPException(status_code=404, detail="Yazı bulunamadı")
    ...
    await db.delete(p)
    db.add(
        AuditLog(actor=user.username, action="blog-delete", message=f"Blog yazısı silindi: {title}")
    )
    await db.commit()
    return None
