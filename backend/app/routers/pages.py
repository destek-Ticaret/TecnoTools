...
def _serialize(p: CmsPage, full: bool = False) -> dict:
    out = {
        ...
        "show_in_footer": p.show_in_footer, "sort_order": p.sort_order,
    }
    if full:
...
async def list_pages_admin(db: AsyncSession = Depends(get_db), _: User = Depends(_can_manage)):
    rows = (await db.execute(select(CmsPage).order_by(CmsPage.sort_order, CmsPage.id))).scalars().all()
...
async def create_page(payload: PageIn, db: AsyncSession = Depends(get_db), user: User = Depends(_can_manage)):
    slug = await _unique_slug(db, slugify(payload.slug or payload.title))
    p = CmsPage(
        slug=slug, title=payload.title, body=payload.body, is_published=payload.is_published,
        show_in_footer=payload.show_in_footer, sort_order=payload.sort_order,
        meta_title=payload.meta_title, meta_description=payload.meta_description,
    )
    db.add(p)
    db.add(AuditLog(actor=user.username, action="page-add", message=f"Sayfa eklendi: {payload.title}"))
    await db.commit()
    await db.refresh(p)
    return _serialize(p, full=True)
...
@router.put("/{page_id}")
async def update_page(page_id: int, payload: PageIn, db: AsyncSession = Depends(get_db), user: User = Depends(_can_manage)):
    ...
    db.add(AuditLog(actor=user.username, action="page-edit", message=f"Sayfa güncellendi: {p.title} (#{p.id})"))
    await db.commit()
    await db.refresh(p)
    return _serialize(p, full=True)
...
async def delete_page(page_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(_can_manage)):
    p = (await db.execute(select(CmsPage).where(CmsPage.id == page_id))).scalar_one_or_none()
    if not p:
        raise HTTPException(status_code=404, detail="Sayfa bulunamadı")
    title = p.title
