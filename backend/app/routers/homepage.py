...
async def list_sections_admin(db: AsyncSession = Depends(get_db), _: User = Depends(_can_manage)):
    rows = (
        (
            await db.execute(
                select(HomepageSection).order_by(HomepageSection.sort_order, HomepageSection.id)
            )
        )
        .scalars()
        .all()
...
async def create_section(
    payload: SectionIn, db: AsyncSession = Depends(get_db), user: User = Depends(_can_manage)
):
    if payload.kind not in VALID_KINDS:
        raise HTTPException(
            status_code=400,
            detail=f"Geçersiz bölüm türü. İzin verilenler: {', '.join(sorted(VALID_KINDS))}",
        )
    ...
    db.add(
        AuditLog(
            actor=user.username,
            action="homepage-add",
            message=f"Ana sayfa bölümü eklendi: {payload.kind}",
        )
    )
    await db.commit()
    await db.refresh(s)
...
async def update_section(
    ...
    s = (
        await db.execute(select(HomepageSection).where(HomepageSection.id == section_id))
    ).scalar_one_or_none()
    if not s:
    ...
    db.add(
        AuditLog(
            ...
            message=f"Ana sayfa bölümü güncellendi (#{s.id})",
        )
    )
    await db.commit()
    await db.refresh(s)
...
async def reorder_sections(
    ...
    rows = (
        (await db.execute(select(HomepageSection).where(HomepageSection.id.in_(order))))
        .scalars()
        .all()
    )
    ...
    db.add(
        AuditLog(
            ...
            message=f"Ana sayfa düzeni yeniden sıralandı ({len(by_id)} bölüm)",
        )
    )
    await db.commit()
...
async def delete_section(
    section_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(_can_manage)
):
    s = (
        await db.execute(select(HomepageSection).where(HomepageSection.id == section_id))
    ).scalar_one_or_none()
    ...
    db.add(
        AuditLog(
            ...
            message=f"Ana sayfa bölümü silindi (#{section_id})",
        )
    )
    await db.commit()
