...
async def subscribe_newsletter(
    request: Request, payload: NewsletterIn, db: AsyncSession = Depends(get_db)
):
...
async def list_subscribers(db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)):
    rows = (
        (await db.execute(select(NewsletterSubscriber).order_by(NewsletterSubscriber.id.desc())))
        .scalars()
        .all()
    )
...
async def delete_subscriber(
    sub_id: int, db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)
):
    row = (
        await db.execute(select(NewsletterSubscriber).where(NewsletterSubscriber.id == sub_id))
    ).scalar_one_or_none()
    if not row:
...
async def list_audit(db: AsyncSession = Depends(get_db), _: User = Depends(require_admin)):
    return (
        (await db.execute(select(AuditLog).order_by(AuditLog.id.desc()).limit(500))).scalars().all()
    )
...
async def list_stock_movements(
    ...
    rows = (
        (await db.execute(select(StockMovement).order_by(StockMovement.id.desc()).limit(500)))
        .scalars()
        .all()
    )
    return [
...
async def clear_stock_movements(
    ...
    db.add(
        AuditLog(
            actor=user.username,
            action="stock-clear",
            message=f"Stok hareketleri temizlendi ({n} kayıt)",
        )
    )
    await db.commit()
    return None
...
async def delete_stock_movement(
    movement_id: int, db: AsyncSession = Depends(get_db), user: User = Depends(require_admin)
):
    row = (
        await db.execute(select(StockMovement).where(StockMovement.id == movement_id))
    ).scalar_one_or_none()
    if not row:
