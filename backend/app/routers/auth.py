...
async def refresh(request: Request, payload: RefreshRequest, db: AsyncSession = Depends(get_db)):
    ...
    db.add(RefreshToken(user_id=user.id, token_hash=refresh_hash, expires_at=expiry))
    await db.commit()
    return TokenPair(access_token=create_access_token(user.username, user.role), refresh_token=raw_refresh)
...
async def forgot_password(
    ...
    user = (
        await db.execute(select(User).where((User.username == payload.username) | (User.email == payload.username)))
    ).scalar_one_or_none()
