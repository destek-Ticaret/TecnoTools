    ...
    for p in rows:
        ...
        ws.append(
            [
                ...
                price,
                (float(p.old_price) if p.old_price else None),
                cost,
                margin,
                p.stock,
...
async def export_returns(db: AsyncSession = Depends(get_db), _: User = Depends(require_editor)):
    rows = (
        (await db.execute(select(ReturnRequest).order_by(ReturnRequest.id.desc()))).scalars().all()
    )
    wb = _make_workbook()
    ws = wb.active
    ws.title = "İadeler"
    _style_header(
        ws, ["İade #", "Sipariş", "Müşteri", "E-posta", "Sebep", "Tutar", "Durum", "Tarih"]
    )
    for r in rows:
        ws.append(
            [
