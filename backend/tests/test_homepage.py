...
async def test_list_sections_filters_inactive(client, auth_client):
    a = await auth_client.post("/api/homepage", json={"kind": "hero", "title": "A", "is_active": True})
    b = await auth_client.post("/api/homepage", json={"kind": "hero", "title": "B", "is_active": False})
    r = await client.get("/api/homepage")
...
async def test_admin_list_includes_inactive(auth_client):
    a = await auth_client.post("/api/homepage", json={"kind": "html", "title": "A", "is_active": True})
    b = await auth_client.post("/api/homepage", json={"kind": "html", "title": "B", "is_active": False})
...
async def test_create_section_ok(auth_client):
    r = await auth_client.post("/api/homepage", json={
        "kind": "product_carousel", "title": "Yeni Gelenler",
        "config": {"category_id": 5, "limit": 12}, "sort_order": 2,
    })
...
async def test_update_section_ok(auth_client):
    ...
    sid = a.json()["id"]
    r = await auth_client.put(f"/api/homepage/{sid}", json={
        "kind": "html", "title": "Yeni", "config": {"html": "<h1>Merhaba</h1>"}, "sort_order": 0,
    })
    assert r.status_code == 200
