...
async def test_list_pages_footer_filter(client, auth_client):
    ...
    await auth_client.post(
        "/api/pages",
        json={
            "title": "Yayında ama footer'da yok",
            "body": "...",
            "is_published": True,
...
async def test_create_page_auto_slug(auth_client):
    cr = await auth_client.post(
        "/api/pages", json={"title": "Sıkça Sorulan Sorular", "body": "..."}
    )
...
async def test_admin_list_includes_drafts(auth_client):
    pub = await auth_client.post(
        "/api/pages",
        json={"title": "Pub", "body": "...", "is_published": True, "show_in_footer": True},
    )
    draft = await auth_client.post(
        "/api/pages", json={"title": "Draft", "body": "...", "is_published": False}
    )
