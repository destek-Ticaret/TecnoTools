...
async def test_permission_catalog_and_enforcement(auth_client):
    ...
    r = await auth_client.post("/api/admin/users", json={"username": "ed1", "password": "parola12345", "role": "editor"})
    uid = r.json()["id"]
    r = await auth_client.put(f"/api/admin/users/{uid}/permissions", json={"permissions": {"content.blog": False}})
    assert r.status_code == 200
    assert "content.blog" not in r.json()["effective"]

    # Editor olarak giriş yap — blog yazma 403, banner yazma 201
    lr = await auth_client.post("/api/auth/login", json={"username": "ed1", "password": "parola12345"})
...
async def test_admin_cannot_get_override(auth_client):
    ...
    r = await auth_client.put(f"/api/admin/users/{admin_id}/permissions", json={"permissions": {"orders.view": False}})
...
async def test_banner_crud_and_public(auth_client):
    r = await auth_client.post("/api/banners", json={"image_url": "http://x/a.jpg", "title": "Yaz", "position": "hero"})
    assert r.status_code == 201
    bid = r.json()["id"]
    # Geçersiz pozisyon
    r = await auth_client.post("/api/banners", json={"image_url": "http://x/b.jpg", "position": "xxx"})
    assert r.status_code == 400
    # Public list
    r = await auth_client.get("/api/banners?position=hero")
    assert r.status_code == 200 and len(r.json()) == 1
    # Update + reorder + delete
    assert (await auth_client.put(f"/api/banners/{bid}", json={"image_url": "http://x/a2.jpg", "position": "strip"})).status_code == 200
    assert (await auth_client.post("/api/banners/reorder", json={"order": [bid]})).status_code == 200
...
async def test_blog_slug_and_views(auth_client):
    r = await auth_client.post("/api/blog", json={"title": "İlk Yazı", "body": "<p>m</p>", "is_published": True})
    assert r.status_code == 201
    slug = r.json()["slug"]
    assert slug == "ilk-yazi"
    # Aynı başlık → benzersiz slug
    r2 = await auth_client.post("/api/blog", json={"title": "İlk Yazı", "body": "<p>m</p>", "is_published": True})
    ...
    d = await auth_client.post("/api/blog", json={"title": "Taslak", "body": "x", "is_published": False})
...
async def test_cms_pages(auth_client):
    r = await auth_client.post("/api/pages", json={"title": "Hakkımızda", "body": "<p>biz</p>", "show_in_footer": True})
    assert r.status_code == 201
    slug = r.json()["slug"]
...
async def test_pricing_rule_percent_and_idempotent_margin(auth_client):
    ...
    # margin %50 → 90
    r = await auth_client.post("/api/pricing-rules", json={"name": "marj", "strategy": "margin", "value": 50})
    rid = r.json()["id"]
...
async def test_pricing_rule_validation(auth_client):
    ...
    r = await auth_client.post("/api/pricing-rules", json={"name": "x", "scope_type": "category", "strategy": "percent", "value": 5})
    assert r.status_code == 400
    # geçersiz strateji
    r = await auth_client.post("/api/pricing-rules", json={"name": "x", "strategy": "bad", "value": 5})
    assert r.status_code == 400
...
async def test_pricing_min_max_guard(auth_client):
    ...
    r = await auth_client.post("/api/pricing-rules", json={"name": "ind", "strategy": "percent", "value": -50, "min_price": 70})
    rid = r.json()["id"]
...
async def test_bulk_import_dry_run_and_commit(auth_client):
    ...
    files = {"file": ("u.xlsx", data, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")}
