...
async def test_list_blog_filters_drafts(client, auth_client):
    pub = await auth_client.post("/api/blog", json={
        "title": "Yayınlanan Yazı", "body": "İçerik...", "is_published": True,
    })
    draft = await auth_client.post("/api/blog", json={
        "title": "Taslak Yazı", "body": "İçerik...", "is_published": False,
    })
...
async def test_list_blog_tag_filter(client, auth_client):
    await auth_client.post("/api/blog", json={
        "title": "Matkap Rehberi", "body": "...", "is_published": True, "tags": ["matkap", "rehber"],
    })
    await auth_client.post("/api/blog", json={
        "title": "Tornavida İpuçları", "body": "...", "is_published": True, "tags": ["tornavida"],
    })
...
async def test_list_blog_limit(client, auth_client):
    for i in range(5):
        await auth_client.post("/api/blog", json={"title": f"Yazı {i}", "body": "...", "is_published": True})
    r = await client.get("/api/blog", params={"limit": 2})
    assert len(r.json()) == 2


async def test_get_blog_increments_view_count(client, auth_client):
    cr = await auth_client.post("/api/blog", json={"title": "Sayım Testi", "body": "...", "is_published": True})
    slug = cr.json()["slug"]
...
async def test_get_blog_404_for_draft(client, auth_client):
    cr = await auth_client.post("/api/blog", json={"title": "Taslak", "body": "...", "is_published": False})
...
async def test_create_blog_auto_slug(auth_client):
    r = await auth_client.post("/api/blog", json={
        "title": "Profesyonel Matkap Seçimi 2026", "body": "İçerik...",
    })
...
async def test_create_blog_sets_published_at(auth_client):
    r = await auth_client.post("/api/blog", json={
        "title": "Yayınlanan", "body": "...", "is_published": True,
    })
    assert r.json()["published_at"] is not None

    r2 = await auth_client.post("/api/blog", json={
...
async def test_update_blog_ok(auth_client):
    ...
    pid = cr.json()["id"]
    r = await auth_client.put(f"/api/blog/{pid}", json={
        "title": "Yeni Başlık", "body": "Yeni içerik", "tags": ["güncel"], "is_published": False,
    })
    assert r.status_code == 200
...
async def test_update_blog_first_publish_stamps_date(auth_client):
    ...
    r = await auth_client.put(f"/api/blog/{pid}", json={
        "title": "Taslak", "body": "...", "is_published": True,
    })
    assert r.json()["published_at"] is not None
...
async def test_admin_list_includes_drafts(auth_client):
    pub = await auth_client.post("/api/blog", json={"title": "Pub", "body": "...", "is_published": True})
    draft = await auth_client.post("/api/blog", json={"title": "Draft", "body": "...", "is_published": False})
