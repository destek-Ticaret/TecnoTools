...
async def test_reservation_reduces_effective_stock_for_others(auth_client):
    pr = await auth_client.post("/api/products", json={
        "name": "Limited", "price": 50, "stock": 10,
    })
    pid = pr.json()["id"]

    # Bir session 3 adet rezerve etsin
    sync = await auth_client.post("/api/reservations/sync", json={
...
async def test_reservation_release(auth_client):
    pr = await auth_client.post("/api/products", json={
        "name": "P", "price": 1, "stock": 5,
    })
    pid = pr.json()["id"]
    await auth_client.post("/api/reservations/sync", json={
        "session_id": "s1", "items": [{"product_id": pid, "qty": 2}],
    })
