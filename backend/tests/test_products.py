...
async def test_create_product_requires_auth(client, seed_admin):
    r = await client.post("/api/products", json={
        "name": "Akülü Matkap", "price": 999.0, "stock": 10,
    })
...
async def test_full_product_lifecycle(auth_client):
    # Yarat
    r = await auth_client.post("/api/products", json={
        "name": "Akülü Matkap",
        "sub": "18V Lityum",
    ...
    # Güncelle
    r2 = await auth_client.put(f"/api/products/{pid}", json={
        "name": "Akülü Matkap", "price": 1299.90, "stock": 20, "icon": "🔧",
    })
...
async def test_inactive_product_hidden_from_public(auth_client):
    r = await auth_client.post("/api/products", json={
        "name": "Stoksuz", "price": 100.0, "stock": 5, "is_active": False,
    })
