...
def test_dev_mock_token_when_creds_missing():
    ...
    result = build_paytr_token(
        order_no="TT-2026-0001", email="x@x.com", amount_kurus=10000,
        user_name="Test User", user_phone="+905551234567", user_address="X",
...
def test_callback_hash_rejects_invalid():
    ...
    ok = verify_callback_hash(
        merchant_oid="TT20260001", status="success",
        total_amount="10000", hash_value="garbage",
    )
    assert ok is False
...
def test_callback_hash_with_credentials(monkeypatch):
    ...
    assert verify_callback_hash(
        merchant_oid="TT20260001", status="success",
        total_amount="10000", hash_value=expected,
    ) is True
