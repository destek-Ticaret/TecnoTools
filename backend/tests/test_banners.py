...
async def test_list_banners_filters_inactive(client, auth_client):
    ...
    a = await auth_client.post("/api/banners", json={
        "title": "Aktif", "image_url": "/a.jpg", "position": "hero",
    })
    b = await auth_client.post("/api/banners", json={
        "title": "Pasif", "image_url": "/b.jpg", "position": "hero", "is_active": False,
    })
...
async def test_list_banners_respects_date_window(client, auth_client):
    ...
    a = await auth_client.post("/api/banners", json={
        "title": "Gelecek", "image_url": "/c.jpg", "position": "strip",
        "starts_at": future,
    })
...
async def test_list_banners_excludes_expired(client, auth_client):
    ...
    a = await auth_client.post("/api/banners", json={
        "title": "Süresi Dolmuş", "image_url": "/d.jpg", "position": "popup",
        "ends_at": past,
    })
...
async def test_create_banner_ok(auth_client):
    r = await auth_client.post("/api/banners", json={
        "title": "Bahar", "subtitle": "Sezon indirimi", "image_url": "/hero.jpg",
        "link_url": "/campaign/bahar", "cta_text": "Keşfet", "position": "hero",
    })
...
async def test_create_banner_invalid_position_400(auth_client):
    r = await auth_client.post("/api/banners", json={
        "title": "X", "image_url": "/x.jpg", "position": "footer",  # izin verilenlerde yok
    })
...
async def test_create_banner_requires_permission(client):
    """Yetkisiz çağrı 401."""
    r = await client.post("/api/banners", json={"title": "X", "image_url": "/x.jpg", "position": "hero"})
    assert r.status_code == 401


async def test_update_banner_ok(auth_client):
    a = await auth_client.post("/api/banners", json={
        "title": "Eski", "image_url": "/x.jpg", "position": "hero", "sort_order": 1,
    })
    bid = a.json()["id"]
    r = await auth_client.put(f"/api/banners/{bid}", json={
        "title": "Yeni", "image_url": "/y.jpg", "position": "strip", "sort_order": 5,
    })
    assert r.status_code == 200
...
async def test_update_banner_404(auth_client):
    r = await auth_client.put("/api/banners/99999", json={
        "title": "X", "image_url": "/x.jpg", "position": "hero",
    })
...
async def test_admin_list_all_includes_inactive(auth_client, client):
    a = await auth_client.post("/api/banners", json={
        "title": "Aktif", "image_url": "/a.jpg", "position": "hero", "is_active": True,
    })
    b = await auth_client.post("/api/banners", json={
        "title": "Pasif", "image_url": "/b.jpg", "position": "hero", "is_active": False,
    })
...
async def test_reorder_banners(auth_client):
    ...
    for i in range(3):
        r = await auth_client.post("/api/banners", json={
            "title": f"B{i}", "image_url": f"/{i}.jpg", "position": "hero",
        })
...
async def test_delete_banner_ok(auth_client):
    a = await auth_client.post("/api/banners", json={
        "title": "Silinecek", "image_url": "/del.jpg", "position": "hero",
    })
    bid = a.json()["id"]
