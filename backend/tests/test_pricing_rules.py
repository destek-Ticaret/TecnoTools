...
async def test_create_rule_ok(auth_client):
    r = await auth_client.post("/api/pricing-rules", json={
...
async def test_create_rule_invalid_scope_400(auth_client):
    r = await auth_client.post("/api/pricing-rules", json={
        "name": "X", "scope_type": "brand",  # geçersiz
    })
...
async def test_create_rule_invalid_strategy_400(auth_client):
    r = await auth_client.post("/api/pricing-rules", json={
        "name": "X", "scope_type": "all", "strategy": "magic",  # geçersiz
    })
    assert r.status_code == 400
...
async def test_category_scope_requires_scope_id(auth_client):
    r = await auth_client.post("/api/pricing-rules", json={
        "name": "X", "scope_type": "category", "scope_id": None, "strategy": "percent", "value": 5,
    })
    assert r.status_code == 400
...
async def test_create_rule_validates_name_length(auth_client):
    r = await auth_client.post("/api/pricing-rules", json={"name": "", "scope_type": "all", "strategy": "percent", "value": 5})
    assert r.status_code == 422  # pydantic min_length=1


async def test_update_rule_ok(auth_client):
    cr = await auth_client.post("/api/pricing-rules", json={
        "name": "Eski", "scope_type": "all", "strategy": "percent", "value": 5,
    })
    rid = cr.json()["id"]
    r = await auth_client.put(f"/api/pricing-rules/{rid}", json={
        "name": "Yeni", "scope_type": "all", "strategy": "fixed", "value": 99.9, "priority": 5,
    })
    assert r.status_code == 200
...
async def test_update_rule_404(auth_client):
    r = await auth_client.put("/api/pricing-rules/99999", json={
        "name": "X", "scope_type": "all", "strategy": "percent", "value": 0,
    })
...
async def test_preview_rule_returns_count(auth_client, db_session):
    ...
    cr = await auth_client.post("/api/pricing-rules", json={
        "name": "Tümünde %20 indirim", "scope_type": "all",
        "strategy": "percent", "value": -20, "priority": 10,
    })
    rid = cr.json()["id"]
...
async def test_apply_rule_changes_prices(auth_client, db_session):
    ...
    cr = await auth_client.post("/api/pricing-rules", json={
        "name": "Tümünde %25 indirim", "scope_type": "all",
        "strategy": "percent", "value": -25, "priority": 1, "is_active": True,
    })
    rid = cr.json()["id"]
...
async def test_delete_rule_ok(auth_client):
    cr = await auth_client.post("/api/pricing-rules", json={
        "name": "Silinecek", "scope_type": "all", "strategy": "percent", "value": 0,
    })
    rid = cr.json()["id"]
